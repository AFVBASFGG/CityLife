...
export class IsoRenderer {
    ...
    rotCoord(x, y) {
        const W = this.state.gridW, H = this.state.gridH;
    ...
    gridToScreen(x, y) {
        ...
        const tw = CONFIG.tileW, th = CONFIG.tileH;
    ...
    drawRoad(x, y) {
        ...
        ctx.beginPath();
        ctx.moveTo(A.x, A.y + th * 0.10);
        ctx.lineTo(B.x - tw * 0.10, B.y);
        ctx.lineTo(C.x, C.y - th * 0.10);
        ctx.lineTo(D.x + tw * 0.10, D.y);
        ctx.closePath();
        ctx.strokeStyle = "rgba(120,140,255,0.28)";
        ctx.stroke();
        ctx.globalAlpha = 1;

        // Determine connectivity for dash orientation and intersection suppression
        const m = this.roadMask(x, y);
        const count = (m.n ? 1 : 0) + (m.s ? 1 : 0) + (m.e ? 1 : 0) + (m.w ? 1 : 0);
        const straightNS = m.n && m.s && !m.e && !m.w;
        const straightEW = m.e && m.w && !m.n && !m.s;

        // Only draw center dashes for straight segments (no corners/intersections/dead-ends)
        // Connectivity-driven markings:
        // - dead-ends: one short connector tick
        // - straight: two ticks (opposite)
        // - corner: two ticks (adjacent)
        ...
        if (dirs.length >= 1 && dirs.length <= 2) {
            ...
            ctx.lineWidth = 2;
            ctx.setLineDash([8, 10]); // subtle

            // Center point
            const cx = p.x, cy = p.y;

            // helper to draw a short tick from center toward an edge midpoint
            const tick = (tx, ty) => {
                const k1 = 0.08, k2 = 0.26; // inner/outer distances along direction
                ctx.beginPath();
    ...
    drawBuilding(id) {
        ...
        const h = (b.type === "house") ? 0.55 :
            (b.type === "park") ? 0.20 :
                (b.type === "mall") ? 0.60 :
                    (b.type === "school") ? 0.65 :
                        (b.type === "hospital") ? 0.75 :
                            (b.type === "office") ? 0.95 :
                                (b.type === "factory") ? 0.85 : 0.6;
    ...
    isoPrism(ctx, cx, cy, tw, th, heightPx, colors, active) {
        ...
        // Determine which faces are "front" based on rotation so it always looks correct.
        // For simplicity, we always draw the two visible vertical faces: (D-C) and (B-C).
        // These are the south-west and south-east faces in screen space.
        ...
        // Optional back face hint (A-B-B2-A2) adds richness without breaking depth
        ...
        // Gloss highlight (pixel-based, subtle)
        ctx.globalAlpha = active ? 0.18 : 0.10;
        ctx.fillStyle = "rgba(255,255,255,1)";
        ctx.beginPath();
        ctx.moveTo(A2.x, A2.y);
        ctx.lineTo(B2.x, B2.y);
        ctx.lineTo(B2.x, lerp(B2.y, B.y, 0.30));
        ctx.lineTo(A2.x, lerp(A2.y, A.y, 0.30));
        ctx.closePath();
        ctx.fill();
